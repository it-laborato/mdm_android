package org.laborato.mdmlab.launcher.ui;
...
import org.laborato.mdmlab.launcher.databinding.ActivityMainBinding;
import org.laborato.mdmlab.launcher.databinding.DialogAccessibilityServiceBinding;
import org.laborato.mdmlab.launcher.databinding.DialogAdministratorModeBinding;
import org.laborato.mdmlab.launcher.databinding.DialogEnterPasswordBinding;
import org.laborato.mdmlab.launcher.databinding.DialogFileDownloadingFailedBinding;
import org.laborato.mdmlab.launcher.databinding.DialogHistorySettingsBinding;
import org.laborato.mdmlab.launcher.databinding.DialogManageStorageBinding;
import org.laborato.mdmlab.launcher.databinding.DialogMiuiPermissionsBinding;
import org.laborato.mdmlab.launcher.databinding.DialogOverlaySettingsBinding;
import org.laborato.mdmlab.launcher.databinding.DialogPermissionsBinding;
import org.laborato.mdmlab.launcher.databinding.DialogSystemSettingsBinding;
import org.laborato.mdmlab.launcher.databinding.DialogUnknownSourcesBinding;
...
import org.laborato.mdmlab.launcher.pro.service.CheckForegroundAppAccessibilityService;
import org.laborato.mdmlab.launcher.pro.service.CheckForegroundApplicationService;
import org.laborato.mdmlab.launcher.server.ServerServiceKeeper;
import org.laborato.mdmlab.launcher.server.UnsafeOkHttpClient;
import org.laborato.mdmlab.launcher.service.LocationService;
import org.laborato.mdmlab.launcher.service.PluginApiService;
import org.laborato.mdmlab.launcher.service.StatusControlService;
import org.laborato.mdmlab.launcher.task.GetServerConfigTask;
import org.laborato.mdmlab.launcher.task.SendDeviceInfoTask;
import org.laborato.mdmlab.launcher.util.AppInfo;
import org.laborato.mdmlab.launcher.util.CrashLoopProtection;
import org.laborato.mdmlab.launcher.util.DeviceInfoProvider;
import org.laborato.mdmlab.launcher.util.InstallUtils;
import org.laborato.mdmlab.launcher.util.PreferenceLogger;
import org.laborato.mdmlab.launcher.util.RemoteLogger;
import org.laborato.mdmlab.launcher.util.SystemUtils;
import org.laborato.mdmlab.launcher.util.Utils;
import org.laborato.mdmlab.launcher.worker.SendDeviceInfoWorker;
...
public class MainActivity
        ...
        public void onReceive( Context context, Intent intent ) {
            switch ( intent.getAction() ) {
                ...
                case Const.ACTION_POLICY_VIOLATION:
                    if (isBackground) {
                        // If we're in the background, let's bring Laborato MDM to top and the notification will be raised in onResume
    ...
    private void setDefaultLauncherEarly() {
        ...
        if (BuildConfig.SET_DEFAULT_LAUNCHER_EARLY && config == null && Utils.isDeviceOwner(this)) {
            // At first start, temporarily set Laborato MDM as a default launcher
            // to prevent the user from clicking Home to stop running Laborato MDM
...
//        RemoteLogger.log(this, Const.LOG_DEBUG, "Saved device admin state: " + administratorMode);
    ...
    private void showContent(ServerConfig config ) {
        ...
        if (ProUtils.kioskModeRequired(this)) {
            ...
            if (kioskApp != null && kioskApp.trim().length() > 0 &&
                    // If Laborato MDM itself is set as kiosk app, the kiosk mode is already turned on;
            ...
            if ( config.getBackgroundImageUrl() != null && config.getBackgroundImageUrl().length() > 0 ) {
                if (picasso == null) {
                    ...
                    if (BuildConfig.TRUST_ANY_CERTIFICATE) {
                        ...
                        // and is just a workaround to use Laborato MDM on the LAN
    ...
    // Run default launcher (Laborato MDM) as if the user clicked Home button
