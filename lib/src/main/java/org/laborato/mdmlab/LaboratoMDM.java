...
/**
 * Higher level Laborato MDM integration API incapsulating reconnection to the service and configuration update
 */
public class LaboratoMDM {

    public interface EventHandler {
        // This method is called when Laborato MDM is ready to answer
        // Get your app settings in this method using MDMService.Preferences.get()
        void onLaboratoMDMConnected();
        // This is just an informative method which doesn't need any actions
        // It should be followed by Connected() method shortly
        void onLaboratoMDMDisconnected();
        // Notification about the configuration change
        // Refresh your app settings in this method using MDMService.Preferences.get()
        void onLaboratoMDMConfigChanged();
    }

    private static LaboratoMDM instance;

    public static LaboratoMDM getInstance() {
        if (instance == null) {
            instance = new LaboratoMDM();
        }
        return instance;
    }
    ...
    private BroadcastReceiver configUpdateReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            if (intent.getAction().equals(Const.NOTIFICATION_CONFIG_UPDATED) && eventHandler != null) {
                eventHandler.onLaboratoMDMConfigChanged();
            }
        }
    };
    ...
    /**
     * Connect to Laborato MDM service
     * @param context
     * @param eventHandler
     * @return true if Laborato MDM exists, false otherwise
     */
    ...
    // Get the admin component for applications using "org.laborato.mdmlab" shared user ID
    ...
    public MDMService.ResultHandler resultHandler = new MDMService.ResultHandler() {
        ...
        public void onMDMConnected() {
            ...
            try {
                ...
                // NPE can be here! queryConfig() may return null if Laborato MDM
            ...
            if (eventHandler != null) {
                eventHandler.onLaboratoMDMConnected();
            }
        }

        @Override
        public void onMDMDisconnected() {
            // This may be raised when Laborato MDM launcher is updated or due to a launcher crash
            mdmConnected = false;
            if (mustRun) {
                if (eventHandler != null) {
                    eventHandler.onLaboratoMDMDisconnected();
                }
                new Handler().postDelayed(new MDMReconnectRunnable(), Const.MDMLAB_RECONNECT_DELAY_FIRST);
            }
        }
    };
    ...
    public class MDMReconnectRunnable implements Runnable {
        ...
        public void run() {
            ...
            if (!mdmService.connect(context, resultHandler)) {
                // Retry in 1 minute
                new Handler().postDelayed(this, Const.MDMLAB_RECONNECT_DELAY_NEXT);
            }
        }
    }
